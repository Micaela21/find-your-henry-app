...
const LOGIN_ALUMNO_GOOGLE = gql`
    mutation login($email: String!, $googleId: String) {
        loginGoogleAlumno(email: $email, googleId: $googleId) {
            token
        }
    }
`;
...
const StudentsRegisterForm: React.FC = () => {
    ...
    const [loginAlumnoGoogle] = useMutation(LOGIN_ALUMNO_GOOGLE);
        ...
        onSubmit: async (valores, { resetForm }) => {
            ...
            if (logged !== null) {
                ...
                try {
                    ...
                    axios.post('http://localhost:3001/send-email', dataMail);
    ...
    const handleGoogle = async (res: any) => {
        ...
        if (logged !== null) {
            ...
            try {
                ...
                try {
                    const { data } = await loginAlumnoGoogle({
                        variables: {
                            email: user.email,
                            googleId: user.googleId
                        }
                    });
                    const token = data.loginGoogleAlumno.token;
                    localStorage.setItem('token', token);
